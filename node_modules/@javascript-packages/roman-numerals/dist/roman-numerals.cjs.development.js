...
var toRoman = function toRoman(number) {
  var recurse = function recurse(number, stack, roman) {
    if (roman === void 0) {
      roman = '';
    }
    ...
    var _stack$shift = stack.shift(),
        largest = _stack$shift.value,
        numerals = _stack$shift.numerals; // Each iteration checks how many times the number
    // on top of the stack is divisible by the number
    // to be converted.


    var quotient = Math.floor(number / largest); // Then, a portion of the Roman numeral sequence
    ...
    roman = "" + roman + numerals.repeat(quotient); // In the next iteration, the current remainder of the
    // division corresponds to the number that is to be
    // converted into a Roman numeral.

    var remainder = number % largest; // Finally, the division remainder (new number to be
...
//# sourceMappingURL=roman-numerals.cjs.development.js.map
